...
omarchy_escalock_refresh_widget() {
  ...
  if ! "$omarchy_shell_command" shell rescanPlugins >/dev/null 2>&1; then
    omarchy_escalock_reload_fallback
    return 0
  fi
  [[ $enabled_state != disabled ]] || return 0

  for (( attempt = 0; attempt < 30; attempt++ )); do
    loaded_version=$("$omarchy_shell_command" "$plugin_id" version 2>/dev/null || true)
    if [[ $loaded_version == "$expected_version" ]]; then
      echo "EscaLock bar widget refreshed to version $expected_version."
      return 0
    fi
