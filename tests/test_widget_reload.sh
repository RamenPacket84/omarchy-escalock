...
reset_case
output=$(omarchy_escalock_refresh_widget 2.2.0 andrewbacon.escalock 2>&1)
grep -Fq 'EscaLock bar widget restarted at version 2.2.0.' <<< "$output"
...
grep -Fq 'text: "Check for updates"' "$project_root/plugin/BarWidget.qml"
grep -Fq ': (root.updateRequired ? "Finish update"' \
  "$project_root/plugin/BarWidget.qml"
grep -Fq 'onboardingProcess.command = [onboardingLauncher, "--update"]' \
  "$project_root/plugin/BarWidget.qml"
grep -Fq 'onboardingProcess.command = [onboardingLauncher, "--finish-update"]' \
  "$project_root/plugin/BarWidget.qml"
grep -Fq 'onboardingProcess.command = [onboardingLauncher, "--review-changes"]' \
  "$project_root/plugin/BarWidget.qml"
